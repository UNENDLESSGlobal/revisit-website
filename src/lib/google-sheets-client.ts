...
const normalizeFeedbackEntry = (
  entry: Partial<FeedbackEntry> & { row?: number | string; status?: boolean | string | number | null },
) => ({
...
export async function fetchFeedbackEntries(sheet: FeedbackSheetName) {
  ...
  const data = await parseResponse<
    Array<Partial<FeedbackEntry> & { row?: number | string; status?: boolean | string | number | null }>
  >(response)
...
export async function updateFeedbackStatus(payload: {
  ...
  const data = await parseResponse<
    Partial<FeedbackEntry> & { row?: number | string; status?: boolean | string | number | null }
  >(response)
